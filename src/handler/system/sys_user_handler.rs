...
pub async fn login(
    ...
    match user_result {
        Ok(u) => match u {
            None => {
                add_login_log(rb, req.mobile, 0, "用户不存在".to_string(), agent).await;
            ...
            Some(user) => {
                ...
                if password.ne(&req.password) {
                    add_login_log(rb, req.mobile, 0, "密码不正确".to_string(), agent).await;
                ...
                match JWTToken::new(id, &username, btn_menu).create_token("123") {
                    Ok(token) => {
                        add_login_log(rb, req.mobile, 1, "登录成功".to_string(), agent.clone())
                            .await;
        ...
        Err(err) => {
            add_login_log(rb, req.mobile, 0, "查询用户异常".to_string(), agent).await;
...
pub async fn query_user_menu(req: HttpRequest,data: web::Data<AppState>) -> Result<impl Responder> {
    ...
    //根据id查询用户
    let result = User::select_by_id(rb, 1).await;
